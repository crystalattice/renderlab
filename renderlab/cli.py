...
def parse_control_args(argv: list[str]) -> argparse.Namespace:
    ...
    verify_parser = corpus_commands.add_parser("verify-assets", help="verify manifest hashes against source ZIPs")
    verify_parser.add_argument("manifest", type=Path)
    verify_parser.add_argument("archive_dir", type=Path)
...
def run_control_command(args: argparse.Namespace) -> int:
    try:
        if args.command == "corpus":
            ...
            elif args.corpus_command == "subset":
                summary = generate_subset(args.manifest, args.spec, args.output)
            else:
                summary = verify_archive_assets(args.manifest, args.archive_dir)
                if args.report:
