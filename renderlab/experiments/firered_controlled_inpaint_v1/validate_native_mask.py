...
BASE = 'f1c18b335ce6df469b526ff905373908636b40b2'
...
def validate():
    ...
    baseline = json.loads(subprocess.check_output(
        ['git', 'show', f'{BASE}:renderlab/experiments/firered_controlled_inpaint_v1/api.cloud.resolved.json'],
        cwd=ROOT, text=True,
    ))
    assert set(graph) == set(baseline) | {'15', '16'}
    assert all(graph[k] == baseline[k] for k in baseline if k != '2')
    assert graph['2'] == {'class_type': 'MaskComposite', 'inputs': {
        'destination': ['15', 0], 'source': ['16', 0], 'x': 552, 'y': 424, 'operation': 'add',
    }}
    assert graph['15'] == {'class_type': 'SolidMask', 'inputs': {'value': 0.0, 'width': 1160, 'height': 896}}
    assert graph['16'] == {'class_type': 'SolidMask', 'inputs': {'value': 1.0, 'width': 128, 'height': 128}}
    ...
    generated = MaskComposite.execute(canvas, rectangle, 552, 424, 'add')[0].cpu().numpy()[0]
    raw = (P / 'mask.png').read_bytes()
    assert hashlib.sha256(raw).hexdigest() == '1a6430abf64aa59c03d06acee164075f6481c7520aaa6a47e4bab9c15a3c86c7'
    ...
    for node in editor['nodes']:
        ...
        for field in specs[node['type']]['input_details']:
            socket = next((s for s in node['inputs'] if s['name'] == field['name']), None)
            if socket is not None:
                if socket['link'] is not None:
                    ...
                    assert edge[0] in origin['links'] and edge[5] == origin['type'] == socket['type']
    ...
    return {
        'status': 'PASS', 'base_commit': BASE, 'node_count': len(graph),
        'changed_existing_nodes': ['2'], 'added_nodes': ['15', '16'],
        'all_non_mask_nodes_and_inputs_unchanged': True, 'source_binding_unchanged': True,
        'set_latent_noise_mask_connection_unchanged': True,
        'mask_dimensions': [1160, 896], 'editable_pixels': int(np.count_nonzero(generated)),
        'normalized_mask_pixel_equivalent': True, 'decoded_rgb_bytes_identical': True,
        ...
        'native_implementation_sha256': hashlib.sha256((ROOT / 'comfy_extras/nodes_mask.py').read_bytes()).hexdigest(),
        'editor_roundtrip_equal': True, 'acyclic_all_nodes_reach_raw_save': True,
        'resolved_sha256': hashlib.sha256((P / 'api.cloud.resolved.json').read_bytes()).hexdigest(),
